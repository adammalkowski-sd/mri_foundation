...
def build_sam_vit_h(
    args=None, checkpoint=None, num_classes=1, image_size=1024, pretrain_sam=False
):
    return _build_sam(
        args,
...
def build_sam_vit_l(
    args, checkpoint=None, num_classes=1, image_size=1024, pretrained_sam=False
):
...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        if "sam_vit" in str(checkpoint) and "HR" not in str(
            checkpoint
        ):  # or 'pretrain_encoderonly_mae_publicmri+breast' in str(checkpoint):
            ...
            for k in state_dict:
                ...
                if "pos_embed" in k:
                    ...
                    curr_pos_embed = curr_pos_embed.view(
                        1, curr_token_size, curr_token_size, -1
                    )
                    curr_pos_embed = curr_pos_embed.permute(0, 3, 1, 2)  # [b, c, h, w]
                    curr_pos_embed = F.interpolate(
                        curr_pos_embed,
            ...
            try:
                ...
                new_state_dict = load_from(
                    sam, new_state_dict, image_size, vit_patch_size
                )
                msg = sam.load_state_dict(new_state_dict, strict=False)
            print(msg)
